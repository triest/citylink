 ...
 * instantiate it using its `create` factory method. The `create` factory
 * method of a Tag can have a variable number of arguments; this way you can
 * pass the dependencies that you need to construct a tag object.
 *
 * > Important: each parameter in addition to the body variable for the
 * `create` method must default to null, otherwise
 * > it violates the constraint with the interface; it is recommended to use
 * the {@see Assert::notNull()} method to
 * > verify that a dependency is actually passed.
 *
 * This Factory also features a Service Locator component that is used to pass
 * the right dependencies to the
 * `create` method of a tag; each dependency should be registered as a service
 * or as a parameter.
 *
 * When you want to use a Tag of your own with custom handling you need to call
 * the `registerTagHandler` method, pass the name of the tag and a Fully
 * Qualified Class Name pointing to a class that implements the Tag interface.
 */
final class StandardTagFactory implements TagFactory
{
    ...
    /**
     * @var string[] An array with a tag as a key, and an FQCN to a class that
     *      handles it as an array value.
     */
    ...
    /**
     * @var mixed[] an array representing a simple Service Locator where we can
     *      store parameters and services that can be inserted into the Factory
     *      Methods of Tag Handlers.
     */
    private $serviceLocator = [];

    /**
     * Initialize this tag factory with the means to resolve an FQSEN and
     * optionally a list of tag handlers.
     *
     * If no tag handlers are provided than the default list in the
     * {@see self::$tagHandlerMappings} property is used.
     *
     * @param FqsenResolver $fqsenResolver
     * @param string[]      $tagHandlers
     *
     * @see self::registerTagHandler() to add a new tag handler to the existing
     *      default list.
     */
    public function __construct(
        FqsenResolver $fqsenResolver,
        array $tagHandlers = null
    ) {
    ...
    {
        $matches = [];
        if (!preg_match('/^@('.self::REGEX_TAGNAME.')(?:\s*([^\s].*)|$)/us',
            $tagLine, $matches)
        ) {
            throw new \InvalidArgumentException(
                'The tag "'.$tagLine
    ...
    /**
     * Creates a new tag object with the given name and body or returns null if
     * the tag name was recognized but the body was invalid.
     *
     * @param string      $body
     * @param string      $name
    ...
    private function createTag($body, $name, TypeContext $context)
    {
        ...
        $arguments = $this->getArgumentsForParametersFromWiring(
            ...
            $this->getServiceLocatorWithDynamicParameters($context, $name,
                $body)
        );
     ...
     * (containing a Factory Method `create`).
     *
    ...
    /**
     * Retrieves the arguments that need to be passed to the Factory Method
     * with the given Parameters.
     *
     * @param \ReflectionParameter[] $parameters
     * @param mixed[]                $locator
     *
     * @return mixed[] A series of values that can be passed to the Factory
     *                 Method of the tag whose parameters is provided with this
     *                 method.
     */
    private function getArgumentsForParametersFromWiring($parameters, $locator)
    {
        ...
        foreach ($parameters as $index => $parameter) {
            $typeHint = $parameter->getClass() ? $parameter->getClass()
                ->getName() : null;
     ...
     * 'create' method of the given tag handler class name.
     *
    ...
    /**
     * Returns a copy of this class' Service Locator with added dynamic
     * parameters, such as the tag's name, body and Context.
     *
     * @param TypeContext $context The Context (namespace and aliasses) that
     *                             may be passed and is used to resolve FQSENs.
     * @param string      $tagName The name of the tag that may be passed onto
     *                             the factory method of the Tag class.
     * @param string      $tagBody The body of the tag that may be passed onto
     *                             the factory method of the Tag class.
     *
     * @return mixed[]
     */
     ...
     *
     * @todo this method should be populated once we implement Annotation
     *       notation support.
     *
     * @return bool
     */
