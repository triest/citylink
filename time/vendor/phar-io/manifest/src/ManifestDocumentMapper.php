...
class ManifestDocumentMapper
{
    ...
    public function map(ManifestDocument $document)
    {
        try {
            ...
            $requirements
                = $this->mapRequirements($document->getRequiresElement());
            ...
            throw new ManifestDocumentMapperException($e->getMessage(),
                $e->getCode(), $e);
        } catch (Exception $e) {
            throw new ManifestDocumentMapperException($e->getMessage(),
                $e->getCode(), $e);
    ...
    private function mapRequirements(RequiresElement $requires)
    {
        ...
        try {
            ...
            throw new ManifestDocumentMapperException(
                sprintf('Unsupported version constraint - %s',
                    $e->getMessage()),
                $e->getCode(),
                $e
            );
    ...
    private function mapBundledComponents(ManifestDocument $document)
    {
        ...
        foreach (
            $document->getBundlesElement()->getComponentElements() as
            $componentElement
        ) {
            $collection->add(
