...
class Generator
{
            ...
            if ($methods !== \array_unique($methods)) {
                throw new RuntimeException(
                    \sprintf(
                        ...
                        \implode(', ', $methods),
                        \implode(', ', \array_unique(\array_diff_assoc($methods,
     ...
     * methods of the class mocked. Concrete methods to mock can be specified
    ...
    public function getMockForTrait(
        ...
        return $this->getMockForAbstractClass($className['className'],
            $arguments, $mockClassName, $callOriginalConstructor,
    ...
    private function getObject(
        ...
        if ($callOriginalConstructor && \is_string($type)
            && !\interface_exists($type, $callAutoload)
        ) {
    ...
    private function generateMock(
        ...
        if (\is_array($type)) {
            foreach ($type as $_type) {
                ...
                foreach ($this->getClassMethods($_type) as $method) {
                    ...
                    $methodReflections[$method]
                        = $typeClass->getMethod($method);
                    $methods[] = $method;
                }
            }
        }
        ...
        if (\class_exists($mockClassName['fullClassName'], $callAutoload)) {
            $isClass = true;
        } elseif (\interface_exists($mockClassName['fullClassName'],
            $callAutoload)
        ) {
            $isInterface = true;
        }
            ...
            if ($class->hasMethod('__clone')) {
                ...
                if (!$cloneMethod->isFinal()) {
                    if ($callOriginalClone && !$isInterface) {
                        $cloneTemplate
                            = $this->getTemplate('unmocked_clone.tpl');
                    } else {
                        $cloneTemplate = $this->getTemplate('mocked_clone.tpl');
                    }
                }
        ...
        if (isset($class)) {
            ...
            if ($isInterface && $class->implementsInterface(Traversable::class)
                ...
                $methods = \array_merge($methods,
                    $this->getClassMethods(Iterator::class));
            }
    ...
    private function generateMockClassDeclaration(
        ...
        if ($isInterface) {
            ...
            if (!\in_array($mockClassName['originalClassName'],
                $additionalInterfaces)
            ) {
                $buffer .= ', ';
    ...
    private function generateMockedMethodDefinitionFromExisting(
        ...
        if (\preg_match('#\*[ \t]*+@deprecated[ \t]*+(.*?)\r?+\n[ \t]*+\*(?:[ \t]*+@|/$)#s',
            $method->getDocComment(), $deprecation)
        ) {
    ...
    private function generateMockedMethodDefinition(
        ...
        if (false !== $deprecation) {
            $deprecation
                = "The $className::$methodName method is deprecated ($deprecation).";
        ...
        $template->setVar(
            [
                ...
                'return_type'     => $allowsReturnNull ? '?'.$returnType
                    : $returnType,
                'arguments_count' => !empty($argumentsForCall)
                    ? \substr_count($argumentsForCall, ',') + 1 : 0,
                'class_name'      => $className,
    ...
    {
        return !($method->isConstructor() || $method->isFinal()
            || $method->isPrivate()
    ...
    private function getMethodParameters(
        ...
        foreach ($method->getParameters() as $i => $parameter) {
            ...
            if (!$forCall) {
                ...
                if (!$parameter->isVariadic()) {
                    if ($parameter->isDefaultValueAvailable()) {
                        ...
                        if ($value === null) {
                            $value = \var_export($parameter->getDefaultValue(),
                                true);
                        }
            ...
            $parameters[] = $nullable.$typeDeclaration.$reference.$name
                .$default;
        }
    ...
    {
        $filename = __DIR__.DIRECTORY_SEPARATOR.'Generator'.DIRECTORY_SEPARATOR
            .$template;
