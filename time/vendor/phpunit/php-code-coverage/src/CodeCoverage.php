...
final class CodeCoverage
{
    ...
    public function merge(self $that): void
    {
        $this->filter->setWhitelistedFiles(
            \array_merge($this->filter->getWhitelistedFiles(),
        ...
        foreach ($that->data as $file => $lines) {
            ...
            foreach ($compareLineNumbers as $line) {
                $thatPriority = $this->getLinePriority($that->data[$file],
                    $line);
                $thisPriority = $this->getLinePriority($this->data[$file],
                    $line);

                if ($thatPriority > $thisPriority) {
                    $this->data[$file][$line] = $that->data[$file][$line];
                } elseif ($thatPriority === $thisPriority
                    && \is_array($this->data[$file][$line])
                ) {
                    $this->data[$file][$line] = \array_unique(
                        \array_merge($this->data[$file][$line],
                            $that->data[$file][$line])
                    );
                }
            }
    ...
    private function applyCoversAnnotationFilter(
        ...
        if ($this->checkForUnintentionallyCoveredCode
            && (!$this->currentId instanceof TestCase
                || (!$this->currentId->isMedium()
                    && !$this->currentId->isLarge()))
        ) {
        ...
        if ($this->checkForUnexecutedCoveredCode) {
            $this->performUnexecutedCoveredCodeCheck($data, $linesToBeCovered,
                $linesToBeUsed);
        }
    ...
    private function getLinesToBeIgnored(string $fileName): array
    {
        ...
        foreach (
            \array_merge($tokens->getClasses(), $tokens->getTraits()) as
            $classOrTrait
        ) {
            ...
            && 0 === \strpos($lastMethod['signature'], 'anonymousFunction'));
            ...
            foreach (
                \range($classOrTraitStartLine, $firstMethodStartLine) as $line
            ) {
                $this->ignoredLines[$fileName][] = $line;
            }

            foreach (
                \range($lastMethodEndLine + 1, $classOrTraitEndLine) as $line
            ) {
                $this->ignoredLines[$fileName][] = $line;
            }
        }
    ...
    private function processUnintentionallyCoveredUnits(
        ...
        $unintentionallyCoveredUnits
            = \array_unique($unintentionallyCoveredUnits);
        ...
        foreach (\array_keys($unintentionallyCoveredUnits) as $k => $v) {
            ...
            foreach (
                $this->unintentionallyCoveredSubclassesWhitelist as $whitelisted
            ) {
