...
class TestSuite implements Test, SelfDescribing, IteratorAggregate
{
    ...
    public function addTestSuite($testClass): void
    {
        ...
        if ($testClass instanceof self) {
            ...
            if (!$testClass->isAbstract()
                && $testClass->hasMethod(BaseTestRunner::SUITE_METHODNAME)
            ) {
     ...
     * added, a <code>PHPUnit\Framework\WarningTestCase</code> will be created
     * instead, leaving the current test run untouched.
     *
    ...
    public function addTestFile(string $filename): void
    {
        ...
        $newClasses = \array_diff(\get_declared_classes(),
        ...
        if (!empty($newClasses)) {
            ...
            $this->foundClasses = \array_merge($newClasses,
                $this->foundClasses);
            $this->declaredClasses = \get_declared_classes();
        }
    ...
    public static function createTest(ReflectionClass $theClass, $name): Test
    {
        ...
        $preserveGlobalState
            = \PHPUnit\Util\Test::getPreserveGlobalStateSettings(
            $className,
            $name
        );

        $runTestInSeparateProcess
            = \PHPUnit\Util\Test::getProcessIsolationSettings(
            $className,
            $name
        );

        $runClassInSeparateProcess
            = \PHPUnit\Util\Test::getClassProcessIsolationSettings(
            $className,
            $name
        );
                ...
                if (isset($data)) {
                    ...
                    if ($data instanceof WarningTestCase
                        ...
                        foreach ($data as $_dataName => $_data) {
                            ...
                            if ($backupSettings['backupStaticAttributes']
                                !== null
                            ) {
    ...
    public function run(TestResult $result = null): TestResult
    {
            ...
            foreach ($hookMethods['beforeClass'] as $beforeClassMethod) {
                if ($this->testCase === true
                    ...
                    if ($missingRequirements
                        = \PHPUnit\Util\Test::getMissingRequirements($this->name,
                        $beforeClassMethod)
                    ) {
                        $this->markTestSuiteSkipped(\implode(PHP_EOL,
                            $missingRequirements));
                    }
        ...
        foreach ($hookMethods['afterClass'] as $afterClassMethod) {
            if ($this->testCase === true && \class_exists($this->name, false)
    ...
    public static function isTestMethod(ReflectionMethod $method): bool
    {
        ...
        $annotations
            = \PHPUnit\Util\Test::parseAnnotations($method->getDocComment());
     ...
     */
    protected static function skipTest(
        $class,
        $methodName,
        $message
    ): SkippedTestCase {
     ...
     */
    protected static function incompleteTest(
        $class,
        $methodName,
        $message
    ): IncompleteTestCase {
