...
abstract class TestCase extends Assert implements Test, SelfDescribing
{
    ...
    /**
     * Sets up an expectation for an exception to be raised by the code under
     * test. Information for expected exception class, expected exception
     * message, and expected exception code are retrieved from a given
     * Exception object.
     */
    ...
    public function run(TestResult $result = null): TestResult
    {
        ...
        $runEntireClass = $this->runClassInSeparateProcess
            && !$this->runTestInSeparateProcess;

        if (($this->runTestInSeparateProcess === true
                || $this->runClassInSeparateProcess === true)
            ...
            if ($this->preserveGlobalState) {
                ...
                if (!empty($GLOBALS['__PHPUNIT_BOOTSTRAP'])) {
                    $globals = '$GLOBALS[\'__PHPUNIT_BOOTSTRAP\'] = '
            ...
            $coverage = $result->getCollectCodeCoverageInformation() ? 'true'
                : 'false';
            $isStrictAboutTestsThatDoNotTestAnything
                = $result->isStrictAboutTestsThatDoNotTestAnything() ? 'true'
                : 'false';
            $isStrictAboutOutputDuringTests
                = $result->isStrictAboutOutputDuringTests() ? 'true' : 'false';
            $enforcesTimeLimit = $result->enforcesTimeLimit() ? 'true'
                : 'false';
            $isStrictAboutTodoAnnotatedTests
                = $result->isStrictAboutTodoAnnotatedTests() ? 'true' : 'false';
            $isStrictAboutResourceUsageDuringSmallTests
                = $result->isStrictAboutResourceUsageDuringSmallTests() ? 'true'
                : 'false';
            ...
            $codeCoverageFilter = \var_export(\serialize($codeCoverageFilter),
                true);
        ...
        if (!isset($e)) {
            try {
                if ($this->outputExpectedRegex !== null) {
                    $this->assertRegExp($this->outputExpectedRegex,
                        $this->output);
                } elseif ($this->outputExpectedString !== null) {
                    $this->assertEquals($this->outputExpectedString,
                        $this->output);
                }
            } catch (Throwable $_e) {
                $e = $_e;
     ...
     * at least N times.
     */
    public static function atLeast(int $requiredInvocations
     ...
     * at most N times.
     */
    public static function atMost(int $allowedInvocations
    ...
    public static function returnArgument(int $argumentIndex
    ): ReturnArgumentStub {
     ...
     * @throws \InvalidArgumentException
     */
    protected function createConfiguredMock(
        $originalClassName,
        array $configuration
    ): MockObject {
     ...
     * @throws \InvalidArgumentException
     */
    protected function createPartialMock(
        $originalClassName,
        array $methods
    ): MockObject {
        return $this->getMockBuilder($originalClassName)
     ...
     * @throws \InvalidArgumentException
     */
    protected function createTestProxy(
    ...
    protected function getMockFromWsdl(
        ...
        if ($originalClassName === '') {
            $originalClassName
                = \pathinfo(\basename(\parse_url($wsdlFile)['path']),
                PATHINFO_FILENAME);
        }
        ...
        if (!empty($this->dependencies) && !$this->inIsolation) {
            ...
            foreach ($this->dependencies as $dependency) {
                ...
                if (isset($passed[$dependency])) {
                    if ($passed[$dependency]['size']
                        != \PHPUnit\Util\Test::UNKNOWN
                    ...
                    if ($deepClone) {
                        ...
                        $this->dependencyInput[$dependency]
                            = $deepCopy->copy($passed[$dependency]['result']);
                    } elseif ($shallowClone) {
                        $this->dependencyInput[$dependency]
                            = clone $passed[$dependency]['result'];
                    } else {
                        $this->dependencyInput[$dependency]
    ...
    private function stopOutputBuffering(): void
    {
        ...
        if ($this->outputCallback !== false) {
            $this->output = \call_user_func($this->outputCallback,
                $this->output);
        }
    ...
    private function compareGlobalStateSnapshots(
        ...
        $backupGlobals = $this->backupGlobals === null
            || $this->backupGlobals === true;
        ...
        if ($this->registerMockObjectsFromTestArgumentsRecursively) {
            ...
            foreach ($testArguments as $testArgument) {
                if ($testArgument instanceof MockObject) {
                    ...
                    && !\in_array($testArgument, $visited, true)
                ) {
