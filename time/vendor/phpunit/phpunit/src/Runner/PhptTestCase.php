...
class PhptTestCase implements Test, SelfDescribing
{
    ...
    public function run(TestResult $result = null): TestResult
    {
        ...
        $jobResult = $this->phpUtil->runJob($code,
            $this->stringifyIni($settings));
        $time = Timer::stop();

        if ($result->getCollectCodeCoverageInformation()
            && ($coverage
                = $this->cleanupForCoverage())
        ) {
        ...
        if ($result->allCompletelyImplemented() && $xfail !== false) {
            $result->addFailure($this,
                new IncompleteTestError('XFAIL section but test passes'),
                $time);
        }
    ...
    private function assertPhptExpectation(array $sections, $output): void
    {
        ...
        foreach ($assertions as $sectionName => $sectionAssertion) {
            if (isset($sections[$sectionName])) {
                ...
                $expected = $sectionName === 'EXPECTREGEX'
    ...
    private function runSkip(&$sections, TestResult $result, $settings): bool
    {
        ...
        $jobResult = $this->phpUtil->runJob($skipif,
            $this->stringifyIni($settings));

        if (!\strncasecmp('skip', \ltrim($jobResult['stdout']), 4)) {
            $message = '';
            if (\preg_match('/^\s*skip\s*(.+)\s*/i', $jobResult['stdout'],
                $skipMatch)
            ) {
    ...
    private function parseExternal(&$sections): void
    {
        ...
        foreach ($allowSections as $section) {
            if (isset($sections[$section.'_EXTERNAL'])) {
                ...
                $sections[$section] = \file_get_contents($testDirectory
                    .$externalFilename);
        ...
        $template->setVar(
            [
                ...
                'autoPrependFile'  => \var_export(
                    !empty($settings['auto_prepend_file'])
                        ? $settings['auto_prepend_file'] : false,
                    true
                ),
            ]
        );
