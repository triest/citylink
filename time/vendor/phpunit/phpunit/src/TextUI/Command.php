...
class Command
{
    ...
    public function run(array $argv, $exit = true): int
    {
        ...
        if ($this->arguments['listTestsXml']) {
            return $this->handleListTestsXml($suite,
                $this->arguments['listTestsXml'], $exit);
        }
    ...
    protected function handleArguments(array $argv): void
    {
        ...
        foreach ($this->options[0] as $option) {
            switch ($option[0]) {
                case '--colors':
                    $this->arguments['colors'] = $option[1]
                        ?: ResultPrinter::COLOR_AUTO;
                ...
                case '--generate-configuration':
                    ...
                    print 'Generating phpunit.xml in '.\getcwd().PHP_EOL
                        .PHP_EOL;
                ...
                case '--strict-global-state':
                    $this->arguments['beStrictAboutChangesToGlobalState']
                        = true;
                ...
                case '--disallow-resource-usage':
                    $this->arguments['beStrictAboutResourceUsageDuringSmallTests']
                        = true;
        ...
        if ($this->arguments['loader'] !== null) {
            $this->arguments['loader']
                = $this->handleLoader($this->arguments['loader']);
        }
        ...
        if (isset($this->arguments['configuration'])) {
            ...
            if (isset($phpunitConfiguration['extensionsDirectory'])
                && !isset($this->arguments['noExtensions'])
                && \extension_loaded('phar')
            ) {
                $this->handleExtensions($phpunitConfiguration['extensionsDirectory']);
            }

            if (isset($phpunitConfiguration['columns'])
            ...
            if (!isset($this->arguments['testsuite'])
                && isset($phpunitConfiguration['defaultTestSuite'])
            ) {
                $this->arguments['testsuite']
                    = $phpunitConfiguration['defaultTestSuite'];
            }

            if (!isset($this->arguments['test'])) {
                $testSuite
                    = $configuration->getTestSuiteConfiguration($this->arguments['testsuite']
                    ?? '');
        ...
        if (isset($this->arguments['printer'])
            ...
            $this->arguments['printer']
                = $this->handlePrinter($this->arguments['printer']);
        }
    ...
    protected function handlePrinter($printerClass, $printerFile = '')
    {
        ...
        $outputStream = isset($this->arguments['stderr']) ? 'php://stderr'
            : null;
    ...
    protected function handleVersionCheck(): void
    {
        ...
        $latestVersion
            = \file_get_contents('https://phar.phpunit.de/latest-version-of/phpunit');
...
EOT;
    ...
    private function handleExtensions($directory): void
    {
        ...
        foreach ($facade->getFilesAsArray($directory, '.phar') as $file) {
            if (!\file_exists('phar://'.$file.'/manifest.xml')) {
                $this->arguments['notLoadedExtensions'][] = $file
            ...
            try {
                ...
                $manifest = ManifestLoader::fromFile('phar://'.$file
                    .'/manifest.xml');
    ...
    private function handleListTestsXml(
        TestSuite $suite,
        string $target,
        bool $exit
    ): int {
