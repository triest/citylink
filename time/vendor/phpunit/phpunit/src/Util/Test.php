...
final class Test
{
     ...
     * @var string
     *
     * @todo This constant should be private (it's public because of
     *       TestTest::testGetProvidedDataRegEx)
     */
     ...
     * @return array|bool
     */
    public static function getLinesToBeCovered(
        string $className,
        string $methodName
    ) {
        ...
        return self::getLinesToBeCoveredOrUsed($className, $methodName,
            'covers');
    }
     ...
     * @return array
     */
    public static function getLinesToBeUsed(
        string $className,
        string $methodName
    ): array {
    ...
    public static function getRequirements(
        ...
        if ($count = \preg_match_all(self::REGEX_REQUIRES_OS, $docComment,
            $matches)
        ) {
        ...
        if ($count = \preg_match_all(self::REGEX_REQUIRES_VERSION, $docComment,
            $matches)
        ) {
            foreach (\range(0, $count - 1) as $i) {
        ...
        if ($count = \preg_match_all(self::REGEX_REQUIRES_VERSION_CONSTRAINT,
            $docComment, $matches)
        ) {
            foreach (\range(0, $count - 1) as $i) {
        ...
        if ($count = \preg_match_all(self::REGEX_REQUIRES_SETTING, $docComment,
            $matches)
        ) {
            $requires['setting'] = [];

            foreach (\range(0, $count - 1) as $i) {
                $requires['setting'][$matches['setting'][$i]]
                    = $matches['value'][$i];
            }
        }

        if ($count = \preg_match_all(self::REGEX_REQUIRES, $docComment,
            $matches)
        ) {
            foreach (\range(0, $count - 1) as $i) {
    ...
    public static function getMissingRequirements(
        ...
        if (!empty($required['PHP'])) {
            $operator = empty($required['PHP']['operator']) ? '>='
                : $required['PHP']['operator'];

            if (!\version_compare(PHP_VERSION, $required['PHP']['version'],
                $operator)
            ) {
                $missing[] = \sprintf('PHP %s %s is required.', $operator,
        ...
        if (!empty($required['PHPUnit'])) {
            ...
            $operator = empty($required['PHPUnit']['operator']) ? '>='
                : $required['PHPUnit']['operator'];

            if (!\version_compare($phpunitVersion,
                $required['PHPUnit']['version'], $operator)
            ) {
                $missing[] = \sprintf('PHPUnit %s %s is required.', $operator,
                    $required['PHPUnit']['version']);
            }
        } elseif (!empty($required['PHPUnit_constraint'])) {
            $phpunitVersion
                = new \PharIo\Version\Version(self::sanitizeVersionNumber(Version::id()));
        ...
        if (!empty($required['OSFAMILY'])
            && $required['OSFAMILY'] !== (new OperatingSystem())->getFamily()
        ) {
            $missing[] = \sprintf('Operating system %s is required.',
                $required['OSFAMILY']);
        }

        if (!empty($required['OS'])) {
            $requiredOsPattern = \sprintf('/%s/i',
                \addcslashes($required['OS'], '/'));
            if (!\preg_match($requiredOsPattern, PHP_OS)) {
                $missing[]
                    = \sprintf('Operating system matching %s is required.',
                    $requiredOsPattern);
            }
        }
        ...
        if (!empty($required['setting'])) {
            foreach ($required['setting'] as $setting => $value) {
                if (\ini_get($setting) != $value) {
                    $missing[] = \sprintf('Setting "%s" must be "%s".',
                        $setting, $value);
                }
            }
        }
        ...
        if (!empty($required['extensions'])) {
            foreach ($required['extensions'] as $extension) {
                ...
                if (!\extension_loaded($extension)) {
                    $missing[] = \sprintf('Extension %s is required.',
                        $extension);
                }
            }
        }
        ...
        if (!empty($required['extension_versions'])) {
            foreach ($required['extension_versions'] as $extension => $required)
            {
                ...
                $operator = empty($required['operator']) ? '>='
                    : $required['operator'];

                if ($actualVersion === false
                    || !\version_compare($actualVersion, $required['version'],
                        $operator)
                ) {
                    $missing[] = \sprintf('Extension %s %s %s is required.',
                        $extension, $operator, $required['version']);
                }
            }
        }
     ...
     * @return array|false
     */
    public static function getExpectedException(
        string $className,
        ?string $methodName
    ) {
        $reflector = new ReflectionMethod($className, $methodName);
        ...
        if (\preg_match(self::REGEX_EXPECTED_EXCEPTION, $docComment,
            $matches)
        ) {
            $annotations = self::parseTestMethodAnnotations(
                $className,
    ...
    public static function getProvidedData(
        ...
        $data = self::getDataFromDataProviderAnnotation($docComment, $className,
            $methodName);
     ...
     *
     * @throws Exception when @testWith annotation is defined but cannot be
     *                   parsed
     *
     * @return null|array array when @testWith annotation is defined,
     *                    null when @testWith annotation is omitted
     */
    public static function getDataFromTestWithAnnotation(string $docComment
    ): ?array {
     ...
     * @return array
     */
    public static function getInlineAnnotations(
        string $className,
        string $methodName
    ): array {
        $method = new ReflectionMethod($className, $methodName);
        ...
        $methodLines = \array_slice($code, $startLine,
            $endLine - $startLine + 1);
        $annotations = [];

        foreach ($methodLines as $line) {
            if (\preg_match('#/\*\*?\s*@(?P<name>[A-Za-z_-]+)(?:[ \t]+(?P<value>.*?))?[ \t]*\r?\*/$#m',
                $line, $matches)
            ) {
    ...
    public static function parseAnnotations(string $docBlock): array
    {
        ...
        if (\preg_match_all('/@(?P<name>[A-Za-z_-]+)(?:[ \t]+(?P<value>.*?))?[ \t]*\r?$/m',
            $docBlock, $matches)
        ) {
            $numMatches = \count($matches[0]);

            for ($i = 0; $i < $numMatches; ++$i) {
                $annotations[$matches['name'][$i]][]
     ...
     * @return ?bool
     */
    public static function getErrorHandlerSettings(
        string $className,
        ?string $methodName
    ): ?bool {
     ...
     * @return bool
     */
    public static function getProcessIsolationSettings(
        string $className,
        string $methodName
    ): bool {
        $annotations = self::parseTestMethodAnnotations(
        ...
        return isset($annotations['class']['runTestsInSeparateProcesses'])
            || isset($annotations['method']['runInSeparateProcess']);
    }

    public static function getClassProcessIsolationSettings(
        string $className,
        string $methodName
    ): bool {
        $annotations = self::parseTestMethodAnnotations(
    ...
    private static function getDataFromDataProviderAnnotation(
        ...
        if (\preg_match_all(self::REGEX_DATA_PROVIDER, $docComment, $matches)) {
            ...
            foreach ($matches[1] as $match) {
                ...
                $leaf = \explode('::',
                    \array_pop($dataProviderMethodNameNamespace));
                $dataProviderMethodName = \array_pop($leaf);

                if (empty($dataProviderMethodNameNamespace)) {
                    $dataProviderMethodNameNamespace = '';
                } else {
                    $dataProviderMethodNameNamespace = \implode('\\',
                            $dataProviderMethodNameNamespace).'\\';
                }
                ...
                if (empty($leaf)) {
                    ...
                    $dataProviderClassName = $dataProviderMethodNameNamespace
                        .\array_pop($leaf);
                }
    ...
    private static function cleanUpMultiLineAnnotation(string $docComment
    ): string {
     ...
     * @return array
     */
    private static function resolveElementToReflectionObjects(string $element
    ): array {
        $codeToCoverList = [];
