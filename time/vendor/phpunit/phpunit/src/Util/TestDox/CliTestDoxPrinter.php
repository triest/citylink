...
/**
 * This printer is for CLI output only. For the classes that output to file,
 * html and xml, please refer to the PHPUnit\Util\TestDox namespace
 */
class CliTestDoxPrinter extends ResultPrinter
{
    ...
    public function startTest(Test $test): void
    {
        ...
        if ($test instanceof TestCase) {
            ...
            if (isset($annotations['method']['testdox'][0])) {
                ...
                $testMethod
                    = $this->prettifier->prettifyTestMethod($test->getName(false));
            }
    ...
    public function endTest(Test $test, float $time): void
    {
        ...
        $this->write($this->currentTestResult->toString($this->previousTestResult,
            $this->verbose));
    ...
    private function printNonSuccessfulTestsSummary(int $numberOfExecutedTests
    ): void {
        ...
        foreach ($this->nonSuccessfulTestResults as $testResult) {
            $this->write($testResult->toString($previousTestResult,
                $this->verbose));
