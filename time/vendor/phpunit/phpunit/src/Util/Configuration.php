...
final class Configuration
{
    ...
    public function getPHPConfiguration(): array
    {
        ...
        foreach ($this->xpath->query('php/const') as $const) {
            ...
            $result['const'][$name]['value'] = $this->getBoolean($value,
                $value);
        }
        ...
        foreach (
            ...
            foreach ($this->xpath->query('php/'.$array) as $var) {
                ...
                if ($var->hasAttribute('verbatim')) {
                    $verbatim
                        = $this->getBoolean($var->getAttribute('verbatim'),
                        false);
                    $result[$array][$name]['verbatim'] = $verbatim;
                }

                if ($var->hasAttribute('force')) {
                    $force = $this->getBoolean($var->getAttribute('force'),
                        false);
    ...
    public function handlePHPConfiguration(): void
    {
        ...
        foreach (
            ['var', 'post', 'get', 'cookie', 'server', 'files', 'request'] as
            $array
        ) {
            /*
    ...
    public function getPHPUnitConfiguration(): array
    {
        ...
        if ($root->hasAttribute('ignoreDeprecatedCodeUnitsFromCodeCoverage')) {
            $result['ignoreDeprecatedCodeUnitsFromCodeCoverage']
                = $this->getBoolean(
     ...
     * @return TestSuite
     */
    public function getTestSuiteConfiguration(string $testSuiteFilter = ''
    ): TestSuite {
    ...
    private function getTestSuite(
        ...
        foreach (
            ...
            if (!empty($testSuiteFilter)
                && !\in_array($directoryNode->parentNode->getAttribute('name'),
                    $testSuiteFilter)
            ) {
                continue;
            }
            ...
            if ($directoryNode->hasAttribute('phpVersion')) {
                $phpVersion
                    = (string)$directoryNode->getAttribute('phpVersion');
            }
        ...
        foreach ($testSuiteNode->getElementsByTagName('file') as $fileNode) {
            ...
            if (!empty($testSuiteFilter)
                && !\in_array($fileNode->parentNode->getAttribute('name'),
                    $testSuiteFilter)
            ) {
                continue;
            }
    ...
    /**
     * if $value is 'false' or 'true', this returns the value that $value
     * represents. Otherwise, returns $default, which may be a string in rare
     * cases. See
     * PHPUnit\Util\ConfigurationTest::testPHPConfigurationIsReadCorrectly
     *
     * @param string      $value
    ...
    private function toAbsolutePath(
        string $path,
