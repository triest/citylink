...
class AbstractPhpProcessTest extends TestCase
{
    ...
    public function testShouldUseGivenSettingsToCreateCommand(): void
    {
        ...
        $expectedCommandFormat
            = '%s -d %callow_url_fopen=1%c -d %cauto_append_file=%c -d %cdisplay_errors=1%c';
        $actualCommand = $this->phpProcess->getCommand($settings);

        $this->assertStringMatchesFormat($expectedCommandFormat,
            $actualCommand);
    }
    ...
    public function testShouldHaveFileToCreateCommand(): void
    {
        ...
        $expectedCommandFormat = \sprintf('%%s -%%c %1$sfile.php%1$s',
            $argumentEscapingCharacter);
