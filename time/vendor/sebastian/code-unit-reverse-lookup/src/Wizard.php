...
class Wizard
{
    ...
    {
        foreach (
            array_merge(get_declared_classes(), get_declared_traits()) as
            $classOrTrait
        ) {
    ...
    private function processFunctionOrMethod(
        ...
        if ($functionOrMethod instanceof \ReflectionMethod) {
            $name = $functionOrMethod->getDeclaringClass()->getName().'::'
                .$name;
        }
        ...
        foreach (
            range($functionOrMethod->getStartLine(),
                $functionOrMethod->getEndLine()) as $line
        ) {
