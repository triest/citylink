...
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends TestCase
{
     ...
     */
    public function testIntegrationUsingPHPFileInVendorGitApply(
        string $fileFrom,
        string $fileTo
    ): void {
        ...
        $diff
            = (new Differ(new StrictUnifiedDiffOutputBuilder([
            'fromFile' => 'Original',
            'toFile'   => 'New',
        ])))->diff($from, $to);
     ...
     * @dataProvider provideFilePairs
     */
    public function testIntegrationUsingPHPFileInVendorPatch(
        string $fileFrom,
        string $fileTo
    ): void {
        $from = FileUtils::getFileContent($fileFrom);
     ...
     */
    public function testIntegrationOfUnitTestCasesGitApply(
        string $expected,
        string $from,
        string $to
    ): void {
     ...
     * @dataProvider provideBasicDiffGeneration
     */
    public function testIntegrationOfUnitTestCasesPatch(
        string $expected,
        string $from,
        string $to
    ): void {
    ...
    public function provideFilePairs(): array
    {
        ...
        foreach ($fileIterator as $file) {
            ...
            $cases[\sprintf("Diff file:\n\"%s\"\nvs.\n\"%s\"\n",
                \realpath($fromFile), \realpath($toFile))]
                = [$fromFile, $toFile];
            $fromFile = $toFile;
        }
     ...
     * @dataProvider provideBasicDiffGeneration
     */
    public function testIntegrationDiffOutputBuilderVersusDiffCommand(
        string $diff,
        string $from,
        string $to
    ): void {
        ...
        $p = new Process(\sprintf('diff -u %s %s',
            \escapeshellarg($this->fileFrom), \escapeshellarg($this->fileTo)));
        $p->run();
        $this->assertSame(1,
            $p->getExitCode()); // note: Process assumes exit code 0 for `isSuccessful`, however `diff` uses the exit code `1` for success with diff

        $output = $p->getOutput();

        $diffLines = \preg_split('/(.*\R)/', $diff, -1,
            PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        $diffLines[0] = \preg_replace('#^\-\-\- .*#', '--- /'.$this->fileFrom,
            $diffLines[0], 1);
        $diffLines[1] = \preg_replace('#^\+\+\+ .*#', '+++ /'.$this->fileFrom,
            $diffLines[1], 1);
        $diff = \implode('', $diffLines);

        $outputLines = \preg_split('/(.*\R)/', $output, -1,
            PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        $outputLines[0] = \preg_replace('#^\-\-\- .*#', '--- /'.$this->fileFrom,
            $outputLines[0], 1);
        $outputLines[1] = \preg_replace('#^\+\+\+ .*#', '+++ /'.$this->fileFrom,
            $outputLines[1], 1);
    ...
    private function doIntegrationTestGitApply(
        string $diff,
        string $from,
        string $to
    ): void {
    ...
    private function doIntegrationTestPatch(
        string $diff,
        string $from,
        string $to
    ): void {
    ...
    private static function setDiffFileHeader(
        string $diff,
        string $file
    ): string {
        $diffLines = \preg_split('/(.*\R)/', $diff, -1,
            PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        $diffLines[0] = \preg_replace('#^\-\-\- .*#', '--- /'.$file,
            $diffLines[0], 1);
        $diffLines[1] = \preg_replace('#^\+\+\+ .*#', '+++ /'.$file,
            $diffLines[1], 1);
