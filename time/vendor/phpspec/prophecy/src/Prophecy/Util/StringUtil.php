...
class StringUtil
{
    ...
    public function stringify($value, $exportObject = true)
    {
        if (is_array($value)) {
            ...
            return '['.implode(', ',
                    array_map(function ($item, $key) use ($stringify) {
        ...
        if (is_object($value)) {
            return $exportObject ? ExportUtil::export($value)
    ...
    public function stringifyCalls(array $calls)
    {
        ...
        return implode(PHP_EOL, array_map(function (Call $call) use ($self) {
            return sprintf('  - %s(%s) @ %s',
                $call->getMethodName(),
                implode(', ', array_map(array($self, 'stringify'),
                    $call->getArguments())),
                str_replace(GETCWD().DIRECTORY_SEPARATOR, '',
                    $call->getCallPlace())
            );
        }, $calls));
    }
