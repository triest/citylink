...
class CallCenter
{
     ...
     *
     * @return mixed Returns null if no promise for prophecy found or promise
     *               return value.
     *
     * @throws \Prophecy\Exception\Call\UnexpectedCallException If no
     *                                                          appropriate
     *                                                          method prophecy
     *                                                          found
     */
    public function makeCall(
        ...
        if ('__destruct' === $methodName
            || 0 == count($prophecy->getMethodProphecies())
        ) {
            $this->recordedCalls[] = new Call($methodName, $arguments, null,
                null, $file, $line);
        ...
        if (!count($matches)) {
            throw $this->createUnexpectedCallException($prophecy, $methodName,
                $arguments);
        }
        ...
        if ($promise = $methodProphecy->getPromise()) {
            try {
                $returnValue = $promise->execute($arguments, $prophecy,
                    $methodProphecy);
            } catch (\Exception $e) {
                $exception = $e;
            }
    ...
    {
        return array_values(
            array_filter($this->recordedCalls,
                function (Call $call) use ($methodName, $wildcard) {
    ...
    private function createUnexpectedCallException(
        ...
        foreach (
            call_user_func_array('array_merge',
                $prophecy->getMethodProphecies()) as $methodProphecy
        ) {
            $expected[] = sprintf(
                "  - %s(\n".
                "%s\n".
