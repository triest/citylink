...
/**
 * Class mirror.
 * Core doubler class. Mirrors specific class and/or interfaces into class node
 * tree.
 *
...
class ClassMirror
{
        ...
        foreach ($interfaces as $interface) {
            if (!$interface instanceof ReflectionClass) {
                throw new InvalidArgumentException(sprintf(
                    ...
                    is_object($interface) ? get_class($interface).' class'
                        : gettype($interface)
                ));
            }
    ...
    private function reflectMethodToNode(
        ...
        if (version_compare(PHP_VERSION, '7.0', '>=')
            ...
            if ('parent' === $returnTypeLower) {
                $returnType = $method->getDeclaringClass()->getParentClass()
                    ->getName();
            }
    ...
    private function reflectArgumentToNode(
        ...
        $name = $parameter->getName() == '...' ? '__dot_dot_dot__'
            : $parameter->getName();
    ...
    private function isNullable(ReflectionParameter $parameter)
    {
        return $parameter->allowsNull()
            && null !== $this->getTypeHint($parameter);
    }

    private function getParameterClassName(ReflectionParameter $parameter)
    {
        try {
            return $parameter->getClass() ? $parameter->getClass()->getName()
                : null;
